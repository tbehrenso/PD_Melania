...
gg1 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "c18", signaling.exclude = 'MIF')
gg2 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "c29", signaling.exclude = 'MIF')
...
netVisual_bubble(cellchat, sources.use = c('c29','c18'), targets.use = c('c1','c8'),  comparison = c(1, 2), angle.x = 45)

gg1 <- netVisual_bubble(cellchat, sources.use = c('c29','c18'), targets.use = c('c1','c8'),  comparison = c(1, 2), max.dataset = 2,
                        title.name = "Increased signaling in PLX", angle.x = 45, remove.isolate = T)
gg2 <- netVisual_bubble(cellchat, sources.use = c('c29','c18'), targets.use = c('c1','c8'),  comparison = c(1, 2), max.dataset = 1,
...
gg1 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.up, sources.use = c('c29','c18'), targets.use = c('c1','c8'), comparison = c(1, 2),
                        angle.x = 90, remove.isolate = T,title.name = paste0("Up-regulated signaling in ", names(object.list)[2]))
pairLR.use.down = net.down[, "interaction_name", drop = F]
gg2 <- netVisual_bubble(cellchat, pairLR.use = pairLR.use.down, sources.use = c('c29','c18'), targets.use = c('c1','c8'), comparison = c(1, 2),
                        angle.x = 90, remove.isolate = T,title.name = paste0("Down-regulated signaling in ", names(object.list)[2]))
gg1 + gg2

library(wordcloud)
computeEnrichmentScore(net.down, species = 'human', variable.both = TRUE) # enriched in SNCA
computeEnrichmentScore(net.up, species = 'human', variable.both = TRUE) # enriched in PLX
